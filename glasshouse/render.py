...
:root{{--ink:#28231f;--primary:#3386e9;--primary-soft:#8fbcf6;--paper:#f7f4ee}}
...
.grid{{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));align-items:stretch}} .card{{min-height:365px;background:#fff;border:1.5px solid var(--primary);display:grid;grid-template-rows:43% 1fr;overflow:hidden;position:relative;cursor:pointer;transform-origin:50% 70%;transition:box-shadow .18s ease,translate .18s ease}}
...
.modal-card{{width:min(1040px,100%);min-height:min(720px,calc(100dvh - 4rem));display:grid;grid-template-columns:minmax(260px,38%) 1fr;background:#fff;border:2px solid var(--primary);box-shadow:18px 22px 0 rgba(0,0,0,.24);position:relative;overflow:hidden}}
.modal-art{{background:var(--primary);min-height:280px;display:grid;place-items:center;overflow:hidden;position:relative}} .modal-art .texture{{position:absolute;inset:0;height:100%;width:100%}} .modal-copy{{padding:clamp(2rem,6vw,5rem);display:flex;flex-direction:column;justify-content:center}} .modal-question{{color:var(--primary);font:700 .78rem/1.2 monospace;text-transform:uppercase;letter-spacing:.05em}} .modal-headline{{font-size:clamp(2.5rem,6vw,5.8rem);line-height:.88;letter-spacing:-.06em;margin:.4em 0 .32em}} .modal-summary{{font-size:1.05rem;font-weight:700;line-height:1.45;margin:0 0 1.4rem}} .modal-detail{{font-size:clamp(1rem,1.6vw,1.25rem);line-height:1.65;max-width:58ch;margin:0}}
.modal-close{{position:absolute;right:1rem;top:1rem;width:48px;height:48px;border:1.5px solid var(--ink);background:#fff;font-size:1.7rem;cursor:pointer;z-index:2}} .modal-close:focus-visible,.modal-nav button:focus-visible{{outline:3px solid var(--primary);outline-offset:3px}} .modal-nav{{display:flex;gap:.5rem;margin-top:2.5rem}} .modal-nav button{{border:1.5px solid var(--ink);background:#fff;padding:.8rem 1rem;font-weight:700;cursor:pointer}} body.modal-open{{overflow:hidden}}
.provn-watermark{{position:fixed;left:14px;bottom:14px;z-index:20;display:flex;align-items:center;gap:8px;padding:7px 10px;background:rgba(247,244,238,.92);border:1px solid rgba(40,35,31,.18);color:var(--ink);text-decoration:none;box-shadow:3px 4px 0 rgba(40,35,31,.09);backdrop-filter:blur(6px)}} .provn-watermark span{{font:600 9px/1 monospace;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap}} .provn-watermark img{{display:block;width:92px;height:auto}} .provn-watermark:hover{{border-color:var(--primary)}} .provn-watermark:focus-visible{{outline:3px solid var(--primary);outline-offset:3px}}
